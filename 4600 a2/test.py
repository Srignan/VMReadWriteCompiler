...
def readDevice(loopCount, delay):
    ...
    for x in range(loopCount):
        device = open("/dev/pa2_out", "r")  # device name change
        fileContents = device.read()
        device.close()
        print(f"Data read from pa2_out (iteration {x+1}): {fileContents}") # testing print statements
        time.sleep(delay/1000)

def writeDevice(inputString, loopCount, delay):
    print("Writing to Device")
    for x in range(loopCount):
        device = open("/dev/pa2_in", "w")  # device name change
